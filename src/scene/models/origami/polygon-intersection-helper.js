...
static intersect(fig1, fig2) {
    ...
    var fig2a = this.alignPolygon(fig2, fig1);
    if (!this.checkPolygons(fig1, fig2a)) {
      return false;
    }
    var edges = this.edgify(fig1, fig2a);
    var polygons = this.polygonate(edges);
    var filteredPolygons = this.filterPolygons(polygons, fig1, fig2a, "intersect");
    return filteredPolygons;
  }
  
  static alignPolygon(polygon, points) {
  ...
  static checkPolygons(fig1, fig2) {
    var figs = [fig1, fig2];
  ...
  static edgify(fig1, fig2) {
    ...
    for(var i = 0; i < primEdges.length; i++) {
      ...
      for(var j = 0; j < primEdges.length; j++) {
        if (i != j) {
          var interPoints = this.findEdgeIntersection(primEdges[i], primEdges[j]);
          this.addNewPoints(interPoints, points);        
        }
      }
  ...
  static addNewPoints(newPoints, points) {
  ...
  static findEdgeIntersection(edge1, edge2) {
    var x1 = edge1[0].x;
    var x2 = edge1[1].x;
    var x3 = edge2[0].x;
    var x4 = edge2[1].x;
  ...
  static polygonExists(polygon, polygons) {
  ...
  static filterPolygons(polygons, fig1, fig2, mode) {
    var filtered = [];
    var c1, c2;
    var point;
  ...
  static removeSmallPolygons(polygons, minSize) {
    var big = [];
    for (var i = 0; i < polygons.length; i++) {
  ...
  static getPointInsidePolygon(polygon) {
    var point;
  ...
  static findPointInsidePolygon(point, polygon) {
    var cross = 0;
    var edges = this.getEdges(polygon);
    var classify;
    var org, dest;
  ...
  static getMidpoints(edges) {
    var midpoints = [];
    var x, y;
    for (var i = 0; i < edges.length; i++) {
      x = (edges[i][0].x + edges[i][1].x) / 2;
      y = (edges[i][0].y + edges[i][1].y) / 2;
      let classify = this.classifyPoint({x: x, y:y}, edges[i]);
