...
public static intersect(fig1:IPolygon, fig2:IPolygon) {
  ...
  let fig2a = this.alignPolygon(fig2, fig1);
  if (!this.checkPolygons(fig1, fig2a)) {
    return false;
  }
  let edges = this.edgify(fig1, fig2a);
  let polygons = this.polygonate(edges);
  let filteredPolygons = this.filterPolygons(polygons, fig1, fig2a, "intersect");
...
public static edgify(fig1:IPolygon, fig2:IPolygon) {
  ...
  let primEdges = this.getEdges(fig1).concat(this.getEdges(fig2));
  let secEdges = [];  
  //check every edge
  for(let i = 0; i < primEdges.length; i++) {
    ...
    for(let j = 0; j < primEdges.length; j++) {
      if (i != j) {
        let interPoints = this.findEdgeIntersection(primEdges[i], primEdges[j]);
...
public static sortPoints(points: IPolygonPoint[]) {
  let p = points as {x: number, y: number, t: number}[];
  p.sort((a,b) => {
        if (a.t > b.t) return 1;
        if (a.t < b.t) return -1;
      });
  ...
  //2. lines are collinear 
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (classify.loc == "ORIGIN" || classify.loc == "DESTINATION") {
        ...
        classify = classify as {loc: string, t: number};
        let x = +((x1 + classify.t*(x2 - x1)).toFixed(9));
        let y = +((y1 + classify.t*(y2 - y1)).toFixed(9));
...
public static classifyPoint(p: IPolygonPoint, edge: IPolygonPoint[]): IPolygonLabel {
  let ax = edge[1].x - edge[0].x;
  let ay = edge[1].y - edge[0].y;
  let bx = p.x - edge[0].x;
  let by = p.y - edge[0].y;
  let sa = ax * by - bx * ay;
...
public static polarAngle(edge: IPolygonPoint[]): number {
  ...
  let theta = Math.atan(dy/dx)*360/(2*Math.PI);
  if (dx > 0) {
      ...
      //add point to polygon
        ...
        for (let j = 0; j < len; j++) {
          ...
          if (!this.equalEdges(edges[j], edges[currentEdge])) {
            ...
            if (p) {
              let classify = this.classifyPoint(p, [org, dest]) as {loc: string, theta: number};
...
public static filterPolygons(polygons: IPolygonPoint[][], fig1: IPolygon, fig2: IPolygon, mode: string) {
  ...
  let c1, c2;
  let point;
  let bigPolygons = this.removeSmallPolygons(polygons, 0.0001);
...
public static getPointInsidePolygon(polygon: IPolygonPoint[]): IPolygonPoint {
  let point;
  let size = this.getSize(polygon);
  let edges = this.getEdges(polygon);
  let y = size.y.min + (size.y.max - size.y.min) / Math.PI;
  let dy = (size.y.max - size.y.min) / 13;
