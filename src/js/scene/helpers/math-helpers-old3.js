...
export class MathHelpers {
	...
	static makePlaneBetween(points, from, to) {
		...
		let to_point;
		if (to_versor.some((e) => isNaN(e))){
			to_point = points[to[0]];
		}else{
			to_point = points[to[0]].map((element, i) => element + from_norm * to_versor[i]);
		}
	...
	static labelFaces(points, faces, from, plane, tolerance) {
		...
		labels = MathHelpers.selectFacesUntilPlane(points, faces, tolerance, plane, start_faces, labels);  // I think old: Maybe do not consider plane in faces away from "from" (so, not in the way to "to"). This is to not create other unwanted axis, in case where the origami is very folded in 3D. Maybe just in case there is another plane intersection!
...
/* 		if (MathHelpers.allEqual(point_sides,1)){
	...
	static divideFacesIntersectingPlane(points, faces, pattern, plane, tolerance, labels, from){
		...
		// Update planes too inside divideFaces:
