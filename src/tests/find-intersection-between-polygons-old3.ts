export class PolygonIntersectionHelper {
    ...
    public static findIntersectionOfPolygonWithPolygon(polygon1: [number, number][], polygon2: [number, number][]) {
        ...
        const intersectionPolygons: [number, number][][] = [];
        ...
        while (!this.checkIfArrayIsEmpty(startEdges)) {
            ...
            let closedPolygon = false;

            while (!closedPolygon) {

                let intersectionPolygonCoord = undefined;

                // Check point
                const currentPolygon = polygons[currentEdge.polygonId];
                const currentPoint = {polygonId: currentEdge.polygonId, edgeId: currentEdge.edgeId, coord: currentEdge.lineSegment[0]};
                const currentCorner = {polygonId: currentEdge.polygonId, cornerId: currentEdge.edgeId, coords: this.findCornerCoords(currentPolygon, currentEdge.edgeId)};
                ...
                if (this.checkIfPolygonInteriorContainsPoint(otherPolygon, currentPoint.coord) || !this.checkIfArrayIsEmpty(intersectionPoints)) {
                    intersectionPolygonCoord = currentPoint.coord;
                    currentPoints.push(...intersectionPoints);
                }

                let nextEdge = this.findNextEdgeSegment(polygons, currentEdge, currentPoints); // TODO: Update to make it versatile

                closedPolygon = this.checkIfPolygonHasClosed(previousLineSegments, nextEdge.lineSegment);

                if (!closedPolygon && intersectionPolygonCoord !== undefined) {
                    intersectionPolygon.push(intersectionPolygonCoord);
                }
                ...
                if (!closedPolygon) {
                    ...
                    let intersectionPoints = this.findIntersectionBetweenLineSegmentAndPolygonEdgesAndCorners(currentEdge, polygons);

                    while (!this.checkIfArrayIsEmpty(intersectionPoints) && !closedPolygon) {
                        ...
                        intersectionPolygonCoord = intersectionPoint.coord;
                        ...
                        // Find next segment
                        nextEdge = this.findNextEdgeSegment(polygons, currentEdge, [intersectionPoint]); // TODO: Update to make it versatile
                        ...
                        if (!closedPolygon) {
                            intersectionPolygon.push(intersectionPolygonCoord);
                            currentEdge = nextEdge;
                            intersectionPoints = this.findIntersectionBetweenLineSegmentAndPolygonEdgesAndCorners(currentEdge, polygons);
                        }
                    }

                    nextEdge = this.findNextEdgeSegment(polygons, currentEdge, [{polygonId:currentEdge.polygonId, edgeId:currentEdge.edgeId, coord:currentEdge.lineSegment[1]}]); // TODO: Update to make it versatile
                }
    ...
    public static checkIfPolygonHasClosed(previousLineSegments: [number, number][][], nextLineSegment: [number, number][]) {
    ...
    public static findNextEdgeSegment(polygons: Record<number, [number, number][]>, currentEdge: {polygonId: number, edgeId: number, lineSegment: [number, number][]}, intersectionPoints: {polygonId: number, edgeId: number, coord: [number, number]}[]) {
        ...
        if (this.checkIfPointsAreEqual(intersectionPointCoord, currentEdge.lineSegment[1])) {
            ...
            nextEdge = branches[branches.length - 1];
    ...
    // public static checkIfAnyLineSegmentHasTheSameDirectionIsCollinearAndIntersectsLineSegmentInterior(lineSegments: [number, number][][], lineSegment: [number, number][]) {
